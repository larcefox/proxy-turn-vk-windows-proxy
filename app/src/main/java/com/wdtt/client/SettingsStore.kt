...
class SettingsStore(context: Context) {
    ...
    companion object {
        ...
        private val LOCAL_NETWORK_ACCESS = booleanPreferencesKey("local_network_access")
        ...
        private fun <T> getProfileKey(baseKey: Preferences.Key<T>, profile: Int): Preferences.Key<T> {
            ...
            return when (baseKey) {
                ...
                MANUAL_PORTS_ENABLED, NO_DTLS, NO_DNS, IS_WHITELIST, WDTT_LINK_MODE, DETAILED_LOGS, LOCAL_NETWORK_ACCESS -> booleanPreferencesKey(newName) as Preferences.Key<T>
    ...
    val localNetworkAccess: Flow<Boolean> = dataStore.data.map { prefs ->
        val profile = prefs[ACTIVE_PROFILE] ?: 0
        prefs[getProfileKey(LOCAL_NETWORK_ACCESS, profile)] ?: false
    }
    ...
    suspend fun saveLocalNetworkAccess(enabled: Boolean) {
        dataStore.edit { prefs ->
            val profile = prefs[ACTIVE_PROFILE] ?: 0
            prefs[getProfileKey(LOCAL_NETWORK_ACCESS, profile)] = enabled
        }
    }
