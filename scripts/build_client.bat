...
set "GO_CLIENT_DIR=%PROJECT_ROOT%app\src\main\assets\android-client"
set "ANDROID_JNILIBS=%PROJECT_ROOT%app\src\main\jniLibs"

:: 1. Find Android SDK/NDK
call "%PROJECT_ROOT%scripts\find_android_sdk_ndk.bat"
if %errorlevel% neq 0 (
    pause
    exit /b 1
)
:: 2. Find Compilers (API 28+)
...
set "CC_PATH_ARM32=%TOOLCHAIN%\armv7a-linux-androideabi28-clang.cmd"
...
set "CC_PATH_X86_64=%TOOLCHAIN%\x86_64-linux-android28-clang.cmd"
...
go mod download
if %errorlevel% neq 0 (
    echo go mod download FAILED for client!
    pause
    exit /b 1
)
if %errorlevel% neq 0 (
